<template>
    <v-card style="width: 100%; margin: 10px 10px;">
        <!--Player select-->
        <v-row>
            <v-col>
                <v-select
                    :items="names" v-model="pitcherData.name" 
                    @update:modelValue="handlePitcherChange"
                    density="compact" 
                ></v-select>
            </v-col>
        </v-row>
        <v-row style="margin-top: -30px;">
            <!--Player image-->
            <v-col cols="3">
                <v-img
                    :src="pitcherData.img"
                ></v-img>
            </v-col>

            <!--Player stats-->
            <v-col cols="9" class="pr-2">
                <div>
                    <table class="stats-table">
                        <thead>
                            <tr>
                            <th>ERA</th>
                            <th>K/9</th>
                            <th>BB/9</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                            <td>{{ pitcherData.era }}</td>
                            <td>{{ pitcherData.kper9 }}</td>
                            <td>{{ pitcherData.bbper9 }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </v-col>
        </v-row>
    </v-card>
</template>

<script>
import axios from 'axios';

export default {
    data(){
        return{
            defaultData: {name: 'Select Pitcher', img: "data:image/png;base64,",
                   era: '-', kper9: '-', bbper9: '-'},
            pitcherData: {},
            names: ['Justin Verlander', 'Max Scherzer'],
            ids: [1, 2],
        }
    },
    methods: {
        handlePitcherChange() {
            const index = this.names.indexOf(this.pitcherData.name);
            const id = this.ids[index];
            const path = 'http://localhost:5000/get_row/PITCHERS/' + id;

            axios.get(path)
                .then((res) => {
                    console.log(res.data)
                    this.pitcherData = res.data;
                })
                .catch((error) => {
                    console.error(error);
                    this.pitcherData = this.defaultData;
                });

            this.$store.commit('setPitcher', {
                id: id,
                name: this.pitcherData.name,
                img: this.pitcherData.img
            });
        },
    },
    created() {
        this.pitcherData = this.defaultData;
    },
};
</script>

<style>
.stats-table {
  border-collapse: collapse;
  width: 95%;
  font-size: 12px;
  margin-bottom: 8px;
}

.stats-table th, .stats-table td {
  border: 1px solid #ddd;
  text-align: left;
}

.stats-table th {
  background-color: #f2f2f2;
}
</style>