<template>
    <v-card style="width: 100%; margin: 10px 10px;">
        <!--Player select-->
        <v-row>
            <v-col>
                <v-select
                    :items="names" v-model="batterData.name" 
                    @update:modelValue="handleBatterChange"
                    density="compact" 
                ></v-select>
            </v-col>
        </v-row>
        <v-row style="margin-top: -30px;">
            <!--Player image-->
            <v-col cols="3">
                <v-img
                    :src="batterData.img"
                ></v-img>
            </v-col>

            <!--Player stats-->
            <v-col cols="9" class="pr-2">
                <div>
                    <table class="stats-table">
                        <thead>
                            <tr>
                            <th>HR</th>
                            <th>OB%</th>
                            <th>SLG%</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                            <td>{{ batterData.hr }}</td>
                            <td>{{ batterData.obp }}</td>
                            <td>{{ batterData.slg }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </v-col>
        </v-row>
    </v-card>
</template>

<script>
import axios from 'axios';

export default {
    data(){
        return{
            defaultData: {name: 'Select Batter', img: "data:image/png;base64,",
                   hr: '-', obp: '-', slg: '-'},
            batterData: {},
            names: ['Mike Trout', 'Shohei Ohtani'],
            ids: [1, 2],
        }
    },
    methods: {
        handleBatterChange() {
            const index = this.names.indexOf(this.batterData.name);
            const id = this.ids[index];
            const path = 'http://localhost:5000/get_row/BATTERS/' + id;

            axios.get(path, { params: { id: id } })
                .then((res) => {
                    console.log(res.data)
                    this.batterData = res.data;
                })
                .catch((error) => {
                    console.error(error);
                    this.batterData = this.defaultData;
                });

            this.$store.commit('setBatter', {
                id: id,
                name: this.batterData.name,
                img: this.batterData.img
            });
        },
    },
    created() {
        this.batterData = this.defaultData;
    },
}
</script>

<style>
.stats-table {
  border-collapse: collapse;
  width: 95%;
  font-size: 12px;
  margin-bottom: 8px;
}

.stats-table th, .stats-table td {
  border: 1px solid #ddd;
  text-align: left;
}

.stats-table th {
  background-color: #f2f2f2;
}
</style>