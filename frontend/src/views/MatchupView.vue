...
export default {
  ...
  updatePitcherStats() {
    ...
    axios.get('http://' + this.$store.state.host + ':5000/api/player_pitching_stats', { params: { player_name: this.pitcherId } })
      .then(response => {
  ...
  updateBatterStats() {
    ...
    axios.get('http://' + this.$store.state.host + ':5000/api/player_batting_stats', { params: { player_name: this.batterId } })
      .then(response => {
        if (!response.data || Object.keys(response.data).length === 0) {
