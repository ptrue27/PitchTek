  ...
  export default {
    ...
    data() {
      ...
      pitcherOutings: [],
      placeholderImageUrl: 'C:/Users/davis/PitchTek-2/frontend/src/assets/strikezone.jpg',
      images: [
      'C:/Users/davis/PitchTek-2/frontend/src/assets/default_heat_map.jpg',
      'C:/Users/davis/PitchTek-2/frontend/src/assets/default_heat_map.jpg',
    ],
      selectedPlayer: null,
  ...
  const nameIndexMap = new Map(); // Continue to use for indices
  ...
  Papa.parse(file, {
    ...
    complete: (results) => {
      ...
      const names = new Set(); // Continue using a Set for unique names

      results.data.forEach((row, index) => {
        const playerName = row.player_name;
        const desWords = row.des ? row.des.split(' ').slice(0, 2).join(' ') : '';
        const imageUrl = row.image_url || 'C:/Users/davis/PitchTek-2/frontend/src/assets/strikezone.jpg'; // Ensure there's an image URL column

        // Handle playerName and add imageURL to the map
        if (playerName) {
          if (!nameIndexMap.has(playerName)) {
            nameIndexMap.set(playerName, { indices: [], imageUrl: imageUrl });
          }
          nameIndexMap.get(playerName).indices.push(index);
          names.add(playerName);
        }

        // Optionally handle desWords if different from playerName
        if (desWords && !nameIndexMap.has(desWords)) {
          nameIndexMap.set(desWords, { indices: [index], imageUrl: imageUrl });
          names.add(desWords);
        }
      });

      this.players = Array.from(names);
      this.nameIndexMap = nameIndexMap; // Updated to store both indices and image URLs
    },
  });
},
